...
@Default
public class CourierChain implements ContractInterface  {
	...
	public Parcel queryParcel(final Context ctx, String parcelId) {
	    // Get the stub from the context
	    ChaincodeStub stub = ctx.getStub();
	    
	    // Fetch the parcel data from the ledger by its ID
	    String parcelData = stub.getStringState(parcelId);
	    
	    // Check if the parcel data is empty, meaning the parcel doesn't exist
	    if (parcelData == null || parcelData.isEmpty()) {
	        // Log the error message
	        String errorMessage = String.format("Parcel %s does not exist", parcelId);
	        System.out.println(errorMessage);

	        // Throw an exception for the missing parcel
	        throw new ChaincodeException(errorMessage, Errors.PARCEL_NOT_FOUND.toString());
	    }

	    // Deserialize the parcel data into a Parcel object
	    Parcel parcel = genson.deserialize(parcelData, Parcel.class);
